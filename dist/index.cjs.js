...
function initializeNachocode(apiKey, options) {
    return new Promise((resolve, reject) => {
        if (!window.Nachocode || !window.Nachocode.env) {
            return reject(new Error('[Nachocode] SDK is not available on the window object.'));
        }
        try {
            ...
            reject(new Error('[Nachocode] An error occurred during SDK initialization.'));
        }
    });
}
...
const NachoContext = require$$0.createContext(undefined);
function NachoProvider({ apiKey, options, version, onInitialized, children, }) {
    const [nachocode, setNachocode] = require$$0.useState(null);
    ...
    return (jsxRuntimeExports.jsx(NachoContext.Provider, { value: { Nachocode: nachocode, loading, error }, children: children }));
}
function useNachocode() {
    const context = require$$0.useContext(NachoContext);
    if (!context) {
